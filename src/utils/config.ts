...
// 1.功能: 定义模块级标志变量，跟踪是否允许读取配置
// 2.目的: 统一管控整个配置模块的读取权限
// 3.上下文: 模块加载时初始化，初始为禁止状态
// 4.原因: 防止模块导入阶段过早读取配置，那时环境变量还没设置好
// 5.坑点: 这是模块单例，全局共享状态，多实例运行会互相影响
let configReadingAllowed = false

// 1.功能: 导出允许配置读取的公共函数
// 2.目的: 给外部提供一个开启配置系统的控制点
// 3.上下文: 在应用初始化流程末尾调用，此时环境变量已设置完成
// 4.原因: 必须等所有环境变量设置完才能开放读取，保证配置读到正确值
// 5.坑点: 必须在正确时机调用，调用太早环境变量没设置，调用太晚读配置报错
export function enableConfigs(): void {
  // 1.功能: 判断是否已经允许读取
  // 2.目的: 保证幂等性，重复调用不重复执行
  // 3.上下文: 函数入口第一句检查，重复调用直接返回
  // 4.原因: 初始化多个路径都可能调用，允许多次调用更易用
  // 5.坑点: 如果第一次执行一半失败了，第二次不会重试，还是保持已允许状态
  if (configReadingAllowed) {
    // 1.功能: 注释说明幂等性意图
    // 2.目的: 告诉读代码的人为什么要这个判断
    // 3.上下文: 配合if使用，解释意图
    // 4.原因: 代码本身能看到行为，但意图需要说明
    // 5.坑点: 无
    // Ensure this is idempotent
    // 1.功能: 提前返回，不执行后面逻辑
    // 2.目的: 已经允许过了，不需要重复执行
    // 3.上下文: if (configReadingAllowed) 为真时返回
    // 4.原因: 提前返回减少代码嵌套，更清晰
    // 5.坑点: 无
    return
  }
  ...
  // 2.目的: 最后计算整个函数执行耗时
  // 3.上下文: 真正工作开始前记录时间
  // 4.原因: 需要性能统计，帮助诊断配置读取慢问题
  // 5.坑点: Date.now()精度毫秒级，太快的话统计不准但没关系
  const startTime = Date.now()
  // 1.功能: 输出诊断日志，标记开始
  // 2.目的: 追踪执行流程，方便调试出问题在哪一步
  // 3.上下文: 开始干活前记日志
  // 4.原因: 可观测性需要，出问题能通过日志定位
  // 5.坑点: 用NoPII版本保证不日志打印敏感信息
  logForDiagnosticsNoPII('info', 'enable_configs_started')

  // 1.功能: 注释说明标志设计目的
  // 2.目的: 告诉后人为什么需要这个标志
  // 3.上下文: 在设置标志前解释背景
  // 4.原因: 防止误在模块初始化加读取配置，这个机制能抓住错误
  // 5.坑点: 无
  // Any reads to configuration before this flag is set show an console warning
  // to prevent us from adding config reading during module initialization
  // 1.功能: 将标志设为true，正式允许读取配置
  // 2.目的: 打开开关，所有后续get调用放行
  // 3.上下文: 已经走到这里，时机成熟，可以开放了
  // 4.原因: 现在环境变量已设置，初始化完成，允许读没问题了
  // 5.坑点: 打开后不能再关上，一旦允许就是永久允许
  configReadingAllowed = true
  // 1.功能: 注释说明为什么只检查全局配置
  // 2.目的: 告诉读者设计假设，未来重构注意
  // 3.上下文: 在调用getConfig前解释为什么只调一次
  // 4.原因: 当前所有配置都存在同一个全局文件，检查一个就够了
  // 5.坑点: 如果拆分多个配置文件，这里必须改，否则其他配置不验证
  // We only check the global config because currently all the configs share a file
  // 1.功能: 调用getConfig读取并验证全局配置
  // 2.目的: 提前验证配置文件格式正确，有错早发现
  // 3.上下文: 允许读取后立刻验证全局配置
  // 4.原因: 配置错了启动就报错，比运行一半炸掉好
  // 5.坑点: 如果配置文件很大，这里会拖慢启动
  getConfig(
    // 1.功能: 传入全局配置文件路径
    // 2.目的: 告诉getConfig读哪个文件
    // 3.上下文: getConfig第一个参数
    // 4.原因: 需要验证全局配置，getGlobalClaudeFile返回正确路径
    // 5.坑点: 路径错了会当成空配置用默认值
    getGlobalClaudeFile(),
    // 1.功能: 传入默认配置工厂函数
    // 2.目的: 文件不存在或读错时，生成默认配置兜底
    // 3.上下文: getConfig第二个参数
    // 4.原因: 首次运行没文件，需要默认配置起步
    // 5.坑点: 如果createDefault改了，老文件不兼容会出问题
    createDefaultGlobalConfig,
    // 1.功能: 传入throwOnInvalid=true参数
    // 2.目的: 配置格式错了直接抛错，不吞错
    // 3.上下文: getConfig第三个参数
    // 4.原因: 验证阶段就是要发现错误，有错必须抛，不能默认兜底
    // 5.坑点: 用户JSON写错逗号，这里直接启动失败，优点是早发现
    true /* throw on invalid */,
  )

  // 1.功能: 输出完成日志，带上耗时统计
  // 2.目的: 记录完成事件和执行耗时，用于性能分析
  // 3.上下文: 所有工作做完之后打日志
  // 4.原因: 完整追踪从开始到结束耗时，帮助性能优化
  // 5.坑点: 无
  logForDiagnosticsNoPII('info', 'enable_configs_completed', {
    // 1.功能: 计算并传入执行耗时毫秒数
    // 2.目的: 日志记录耗时，供后续分析
    // 3.上下文: 日志元数据字段duration_ms
    // 4.原因: 现在减开始时间得到总耗时
    // 5.坑点: Date.now()是墙上时间，包含其他进程调度，不是精准CPU时间
